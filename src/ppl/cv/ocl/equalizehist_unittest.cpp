...
bool PplCvOclEqualizehistTest::apply() {
  ...
  uchar* input = (uchar*)clEnqueueMapBuffer(queue, gpu_input, CL_TRUE, CL_MAP_WRITE,
  ...
  bool identity0 = checkMatricesIdentity<uchar>((const uchar*)cv_dst.data, cv_dst.rows,
      ...
      epsilon);
  bool identity1 = checkMatricesIdentity<uchar>((const uchar*)cv_dst.data, cv_dst.rows,
      cv_dst.cols, cv_dst.channels(), cv_dst.step, output, cv_dst.cols, 
      epsilon);
