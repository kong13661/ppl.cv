...
bool PplCvOclSplitToTest<T, channels>::apply() {
  ...
  bool identity0 = checkMatricesIdentity<T>((const T*)cv_dst0.data, cv_dst0.rows,
      cv_dst0.cols, cv_dst0.channels(), cv_dst0.step, (const T*)dst0.data, dst0.step,
      epsilon);
  bool identity1 = checkMatricesIdentity<T>((const T*)cv_dst1.data, cv_dst1.rows,
      cv_dst1.cols, cv_dst1.channels(), cv_dst1.step, (const T*)dst1.data, dst1.step,
      epsilon);
  bool identity2 = checkMatricesIdentity<T>((const T*)cv_dst2.data, cv_dst2.rows,
      cv_dst2.cols, cv_dst2.channels(), cv_dst2.step, (const T*)dst2.data, dst2.step,
      epsilon);

  bool identity4 = checkMatricesIdentity<T>((const T*)cv_dst0.data, cv_dst0.rows,
      cv_dst0.cols, cv_dst0.channels(), cv_dst0.step, output0,
      dst0.cols * sizeof(T), epsilon);
  error_code = clEnqueueUnmapMemObject(queue, gpu_output0, output0, 0, NULL,
                                       NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
  bool identity5 = checkMatricesIdentity<T>((const T*)cv_dst1.data, cv_dst1.rows,
      cv_dst1.cols, cv_dst1.channels(), cv_dst1.step, output1,
      dst1.cols * sizeof(T), epsilon);
  error_code = clEnqueueUnmapMemObject(queue, gpu_output1, output1, 0, NULL,
                                       NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
  bool identity6 = checkMatricesIdentity<T>((const T*)cv_dst2.data, cv_dst2.rows,
      cv_dst2.cols, cv_dst2.channels(), cv_dst2.step, output2,
      dst2.cols * sizeof(T), epsilon);
  error_code = clEnqueueUnmapMemObject(queue, gpu_output2, output2, 0, NULL,
                                       NULL);
  CHECK_ERROR(error_code, clEnqueueUnmapMemObject);

  bool identity3, identity7;
  if (channels == 4) {
    identity3 = checkMatricesIdentity<T>((const T*)cv_dst3.data, cv_dst3.rows,
        cv_dst3.cols, cv_dst3.channels(), cv_dst3.step, (const T*)dst3.data, dst3.step,
        epsilon);
    identity7 = checkMatricesIdentity<T>((const T*)cv_dst3.data, cv_dst3.rows,
        cv_dst3.cols, cv_dst3.channels(), cv_dst3.step, output3,
        dst3.cols * sizeof(T), epsilon);
    error_code = clEnqueueUnmapMemObject(queue, gpu_output3, output3, 0, NULL,
                                        NULL);
    CHECK_ERROR(error_code, clEnqueueUnmapMemObject);
  }
  ...
  if (channels == 3) 
    return (identity0 && identity1 && identity2 && identity4 && identity5 && identity6);
  if (channels == 4) 
    return (identity0 && identity1 && identity2 && identity3 && identity4 && identity5 && identity6 && identity7);
}
