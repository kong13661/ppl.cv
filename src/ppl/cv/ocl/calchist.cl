...
__kernel void unmaskCalchistKernel0(global const uchar* src, const int cols, 
    global int* hist) {
...
__kernel void unmaskCalchistKernel1(global const uchar* src, int src_stride,
    ...
    int count = local_hist[index_hist];
        // atomic_add(&hist[index_hist], 10);
    if (count > 0){
        atomic_add(&hist[index_hist], count);
    }
}

__kernel void maskCalchistKernel0(global const uchar* src, global const uchar* mask, 
...
__kernel void maskCalchistKernel1(global const uchar* src, int src_stride,
